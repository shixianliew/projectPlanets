...
jsPsych.plugins['inference-check-4'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    html += '">';
    html += '<input type="range" value="'+trial.start+'" min="'+trial.min+'" max="'+trial.max+'" step="'+trial.step+'" style="width: 100%;" id="jspsych-inference-check-4-response5"></input>';
    html += '<div>'
    for(var j=0; j < trial.labels_top.length; j++){
      var width = 100/(trial.labels_top.length-1);
      var left_offset = (j * (100 /(trial.labels_top.length - 1))) - (width/2);
    ...
    html += '">';
    html += '<input type="range" value="'+trial.start+'" min="'+trial.min+'" max="'+trial.max+'" step="'+trial.step+'" style="width: 100%;" id="jspsych-inference-check-4-response7"></input>';
    html += '<div>'
    for(var j=0; j < trial.labels_top.length; j++){
      var width = 100/(trial.labels_top.length-1);
      var left_offset = (j * (100 /(trial.labels_top.length - 1))) - (width/2);
    ...
    if(trial.require_movement){
      display_element.querySelector('#jspsych-inference-check-4-response8').addEventListener('change', function(){
    ...
    display_element.querySelector('#jspsych-inference-check-4-next').addEventListener('click', function() {
      ...
      response.response5 = display_element.querySelector('#jspsych-inference-check-4-response5').value;
      response.response6 = display_element.querySelector('#jspsych-inference-check-4-response6').value;
      response.response7 = display_element.querySelector('#jspsych-inference-check-4-response7').value;
    ...
    function end_trial(){
      ...
      var trialdata = {
        ...
        "rate_2": response.response3,
        "conf_2": response.response4,
        "rate_3": response.response5,
        "conf_3": response.response6,
        "rate_4": response.response7,
